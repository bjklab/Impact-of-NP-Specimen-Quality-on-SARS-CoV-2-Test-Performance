...
m_logit_scale_radiology_brms$data %>%
  ...
  mutate(chest_ct_number = case_when(chest_ct_infiltrates == "Absent" ~ 65,
                                     chest_ct_infiltrates == "Present" ~ 43,
                                     chest_ct_infiltrates == "Unknown" ~ 1174),
...
bd_impute %>%
  mutate(covid_positive = as.numeric(grepl("POS",result))) %>%
  dplyr::select(covid_positive, chest_ct_infiltrates, ms2_ct, bactin_ct, sarscov2_ct) %>%
  mutate_at(.vars = vars(-covid_positive, -chest_ct_infiltrates), .funs = list("flip" = ~ -log(.x)),) %>%
  mutate_at(.vars = vars(-covid_positive, -chest_ct_infiltrates, -contains("flip")), .funs = list("scale" = ~ scale(.x)[,1])) %>%
  mutate(chest_ct_infiltrates = ifelse(is.na(chest_ct_infiltrates), "Unknown", ifelse(chest_ct_infiltrates == TRUE, "Present", "Absent"))) %>%
  filter(chest_ct_infiltrates != "Unknown") %>%
  mutate(chest_ct_infiltrates = chest_ct_infiltrates == "Present") %>%
  identity() -> radiology_mediation_data
radiology_mediation_data



# fit Bayesian mediation model in brms
bayestestR::mediation(model = med_lin, ci = 0.95)



f1 <- bf(covid_positive ~ chest_ct_infiltrates + bactin_ct_scale, family = "bernoulli")
f2 <- bf(chest_ct_infiltrates ~ bactin_ct_scale, family = "bernoulli")
med_log <- brm(f1 + f2 + set_rescor(FALSE),
               data = radiology_mediation_data,
               cores = 4,
               iter = 2000,
               warmup = 1000,
               chains = 4,
...
bayestestR::mediation(model = med_log, ci = 0.95)
sjstats::mediation(sjmisc::std(med_log))
